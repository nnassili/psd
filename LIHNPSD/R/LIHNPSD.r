...
standardfit <- function(d, r, hist, trace, iter, plotqq, weights, merge_tails, do_lambda) UseMethod("standardfit", d)
...
LIHNPSD_standardfit_fn <- function( psd, data_stats, hist, plotqq=1, 
  ...
  if ( plotqq == 1 ) {
    ...
    text(xpos,ypos-9*ydf,labels=paste("lambda", sprintf("%.6f",d$lambda)),cex=sc)
    xmax <- max(qqp$x)
    xpos <- xmax*0.4
...
standardfit.LIHNPSD <- function(d, r, hist, trace= 0, iter=5000, 
  plotqq= 1, weights=list(), merge_tails=c(0,0), do_lambda=FALSE ) {
  ...
  data_stats <- c( mean(r), sqrt(var(r)), skewness(r), kurtosis(r)-3.0 )
  psd <- c( d$location/data_stats[2], d$sigma/data_stats[2], d$alpha, d$gamma, d$beta )
  if ( do_lambda ) psd <- c(psd, d$lambda)
...
TimeSeriesLogReturn <- function( pr, days ) {
  ...
  for (i in 2:length(pr2)) {
    if ( pr2[i-1] > 0.0 && pr2[i] ) { 
      r[i-1] <- log(pr2[i]/pr2[i-1])
    }
  }
  r
}
